...
$dumpFolder = sys_get_temp_dir() . '/pg_dump';
$dumpAllFolder = sys_get_temp_dir() . '/pg_dumpall';
if (!is_dir($dumpFolder)) {
    mkdir($dumpFolder);
}
if (!is_dir($dumpAllFolder)) {
    mkdir($dumpAllFolder);
}
$i = 0;
$services = $serviceManager->getAllServices();
foreach ($services as $service) {
    $pgConfig = $databaseConnector->parseDbFromService($service);
    if (strtolower($pgConfig['type']) !== 'pgsql') {
        continue;
    }
    $conf['servers'][$i]['desc'] = $service->getName();
    /* Authentication type */
    /* Server parameters */
    $conf['servers'][$i]['host'] = $pgConfig['host'];
    $conf['servers'][$i]['port'] = $pgConfig['port'];
    $conf['servers'][$i]['defaultdb'] = $pgConfig['path'];
    $conf['servers'][$i]['sslmode'] = 'allow';
    $conf['servers'][$i]['pg_dump_path'] = $dumpFolder;
    $conf['servers'][$i]['pg_dumpall_path'] = $dumpAllFolder;
    $conf['servers'][$i]['theme']['default'] = 'bootstrap';
    $conf['servers'][$i]['theme']['user']['specific_user'] = 'bootstrap';
    $conf['servers'][$i]['theme']['db']['specific_db'] = 'bootstrap';
    $i++;
}
